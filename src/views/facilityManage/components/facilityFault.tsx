...
const FacilityFault = () => {
  ...
  if (["1", "2", "3"].includes(role)) {
    columns.push({
      ...
      render: (record: any) => (
        <Space size="large">
          {role === "3" && (
            <a style={record.approval === "1" ? {} : disabledA} onClick={() => writeRecordVisible(record)}>
              填写维修报告
            </a>
          )}
          {["1", "2"].includes(role) && (
            <>
              ...
              <a onClick={() => showDeleteModal(record.id)} style={{ color: "red" }}>
                删除
              </a>
            </>
          )}
  ...
  return (
    <>
      ...
      />
      <Modal title="请填写维修报告" open={visible} onOk={handleWrite} onCancel={() => setVisible(false)}>
        <Form form={form}>
          <Form.Item name="repairer" label="维修人" rules={[{ required: true, message: "请输入" }]}>
            <Select style={{ width: 120 }} disabled>
              <Select.Option value={userInfo.id}>{userInfo.name}</Select.Option>
            </Select>
          </Form.Item>
          <Form.Item name="equipment" label="维修设备" rules={[{ required: true, message: "请输入" }]}>
            <Select style={{ width: 120 }} disabled>
          ...
          <Form.Item name="cause" label="故障原因" rules={[{ required: true, message: "请输入故障原因" }]}>
            <TextArea placeholder="请输入" />
          </Form.Item>
          <Form.Item name="result" label="维修结果" rules={[{ required: true, message: "请输入维修结果" }]}>
            <TextArea placeholder="请输入" />
          </Form.Item>
        </Form>
      </Modal>
